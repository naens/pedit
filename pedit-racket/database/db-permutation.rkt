...
(define (db-permutation-set db tv-id f-node-id t-node-id)
  (let ((ft-node-id (db-permutation-dest db tv-id f-node-id))
        (tt-node-id (db-permutation-dest db tv-id t-node-id)))
    (when (not (= f-node-id t-node-id))
      (query-exec db "delete from Permutation where TextVersionID=$1 and (TextNodeFromID=$2 or TextNodeFromID=$3);" tv-id f-node-id t-node-id)
      (when (not (= f-node-id tt-node-id))
        (query-exec db "insert into Permutation (TextVersionID, TextNodeFromID, TextNodeToID) values ($1, $2, $3);" tv-id f-node-id tt-node-id))
      (when (not (= t-node-id ft-node-id))
        (query-exec db "insert into Permutation (TextVersionID, TextNodeFromID, TextNodeToID) values ($1, $2, $3);" tv-id t-node-id ft-node-id)))))
